...
onMounted(async () => {
  ...
  try {
    const { data, error } = await supabase.rpc('get_public_quotation', { doc_id: id })
...
// or we could check if branding_snapshot specifies a template in the future.
const activeTemplateComponent = computed(() => {
  return TEMPLATE_VARIANTS[DEFAULT_TEMPLATE].quotation
})
...
<template>
  <div class="min-h-screen bg-slate-100 print:bg-white pb-12">
    ...
    <div class="bg-white border-b sticky top-0 z-10 print:hidden shadow-sm">
      <div class="max-w-[210mm] mx-auto px-4 h-16 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 bg-teal-600 text-white rounded-md flex items-center justify-center font-bold text-lg">
            Q
          </div>
          <div>
            <h1 class="font-bold text-slate-900 leading-tight">Quotation</h1>
        ...
        <div class="flex items-center gap-3">
          ...
          <Button size="sm" @click="handlePrint" :disabled="isLoading || !documentData" class="gap-2 bg-[#0F766E] hover:bg-[#0F766E]/90">
    ...
    <div v-else class="text-center py-20">
      ...
      <p class="text-slate-500 max-w-sm mx-auto">This quotation could not be loaded. It may have been deleted or the link is invalid.</p>
    </div>
  </div>
</template>
