  ...
  <div class="doc-modern" :style="cssVars">
    ...
    <div class="doc-body">
      ...
      <div class="doc-meta-row">
        ...
        <div class="doc-meta-card" v-if="data.expiry_date">
      ...
      <div class="doc-client-card">
        ...
        <div class="doc-card-body">
          <h2 class="doc-card-title">Prepared For</h2>
          <p class="doc-client-name">{{ data.customers?.name || data.customer?.name }}</p>
          <div class="doc-client-details">
            <span v-if="data.customers?.email || data.customer?.email">
              {{ data.customers?.email || data.customer?.email }}
            </span>
            <span v-if="data.customers?.phone || data.customer?.phone">
              {{ data.customers?.phone || data.customer?.phone }}
            </span>
            <span v-if="data.customers?.address || data.customer?.address">
      ...
      <footer class="doc-footer">
        ...
        <p class="doc-footer-message">Thank you for considering our services.</p>
...
.doc-company-name { font-size: 20px; font-weight: 700; color: var(--doc-primary-contrast); margin: 0; letter-spacing: -0.01em; }
...
.doc-meta-card {
  ...
  background: var(--doc-surface, #f9fafb); border: 1px solid var(--doc-border-light, #f3f4f6);
...
/* ── Client Card ── */
.doc-client-card {
  display: flex; margin-bottom: 28px; border-radius: 10px; overflow: hidden;
  border: 1px solid var(--doc-border-light, #f3f4f6); break-inside: avoid;
}
...
.doc-th {
  ...
  padding: 12px 16px; background: var(--doc-primary-8, #f8f9fa); color: var(--doc-primary-80, #374151);
...
.doc-totals-card {
  display: flex; width: 300px; border-radius: 10px; overflow: hidden;
  border: 1px solid var(--doc-border-light, #f3f4f6);
}
...
.doc-total-grand {
  ...
  color: var(--doc-primary); padding-top: 14px; margin-top: 10px;
  border-top: 2px solid var(--doc-primary-15);
}
