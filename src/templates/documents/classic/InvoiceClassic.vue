    ...
    <div class="doc-body">
      ...
      <header class="doc-header">
        ...
        <div class="doc-header-right">
          <div class="doc-type-block">
            <span class="doc-type-text">INVOICE</span>
          </div>
        </div>
      </header>
      ...
      <div class="doc-info-columns">
        <div class="doc-info-left">
          <h2 class="doc-info-heading">Billed To:</h2>
          <p class="doc-client-name">{{ data.customers?.name }}</p>
          <p class="doc-client-line" v-if="data.customers?.email">{{ data.customers?.email }}</p>
          <p class="doc-client-line" v-if="data.customers?.phone">Tel: {{ data.customers?.phone }}</p>
          <p class="doc-client-line" v-if="data.customers?.address">{{ data.customers?.address }}</p>
        </div>
        <div class="doc-info-right">
          <table class="doc-info-table">
            <tr>
              <td class="doc-info-label">Invoice No.</td>
              <td class="doc-info-value">{{ data.number }}</td>
            </tr>
            <tr>
              <td class="doc-info-label">Issue Date</td>
              <td class="doc-info-value">{{ formatDate(data.issue_date) }}</td>
            </tr>
            <tr v-if="data.due_date">
              <td class="doc-info-label">Due Date</td>
              <td class="doc-info-value">{{ formatDate(data.due_date) }}</td>
            </tr>
            <tr>
      ...
      <section class="doc-totals-section">
        <div class="doc-totals-table">
          ...
          <div class="doc-total-row" v-if="data.amount_paid > 0">
            <span class="doc-total-label">Amount Paid</span>
            <span class="doc-total-value">-{{ formatCurrency(data.amount_paid) }}</span>
          </div>
          <div class="doc-total-grand-row">
            <span class="doc-total-grand-label">{{ data.amount_paid > 0 ? 'BALANCE DUE' : 'TOTAL DUE' }}</span>
            <span class="doc-total-grand-value">{{ formatCurrency(data.total - (data.amount_paid || 0)) }}</span>
          </div>
        </div>
      </section>
      ...
      <section v-if="data.notes" class="doc-notes-section">
        <h3 class="doc-notes-heading">Payment Terms</h3>
...
<style scoped>
.doc-classic { font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #1a1a1a; background: #ffffff; min-height: 297mm; width: 100%; font-size: 13px; line-height: 1.5; }
...
.doc-type-block { padding: 10px 28px; background: var(--doc-primary); color: var(--doc-primary-contrast); font-size: 16px; font-weight: 800; letter-spacing: 0.12em; text-align: center; }
...
.doc-th { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; padding: 10px 8px; background: var(--doc-primary); color: var(--doc-primary-contrast); border: 1px solid var(--doc-primary-70, #374151); }
.doc-th-left { text-align: left; } .doc-th-right { text-align: right; }
.doc-row { break-inside: avoid; }

.doc-td { padding: 9px 8px; border-bottom: 1px solid #e5e7eb; border-left: 1px solid #e5e7eb; border-right: 1px solid #e5e7eb; vertical-align: top; }
.doc-td-center { text-align: center; } .doc-td-right { text-align: right; } .doc-td-bold { font-weight: 600; } .doc-td-num { color: #9ca3af; font-size: 12px; }
...
.doc-total-label { font-weight: 500; } .doc-total-value { font-weight: 600; color: #1a1a1a; }
.doc-total-grand-row { display: flex; justify-content: space-between; padding: 12px 14px; background: var(--doc-primary); color: var(--doc-primary-contrast); font-weight: 800; font-size: 16px; }
